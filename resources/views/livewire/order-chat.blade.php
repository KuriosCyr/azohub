<div class="bg-white rounded-3xl shadow-lg overflow-hidden">
    <div class="bg-gradient-to-r from-blue-900 to-blue-800 text-white px-6 py-4">
    ...
    <div id="messages-container" class="h-96 overflow-y-auto p-6 space-y-4 bg-gray-50">
        @forelse($messages as $message)
            ...
            <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }}">
                <div class="flex gap-3 max-w-[70%] {{ $isMine ? 'flex-row-reverse' : '' }}">
                    ...
                    {{-- Message bubble --}}
                    <div>
                        <div class="px-4 py-3 rounded-2xl {{ $isMine ? 'bg-blue-900 text-white' : 'bg-white border border-gray-200 text-gray-900' }}">
                                ...
                                <div class="mt-3 space-y-2">
                                    @foreach($message->attachments as $index => $file)
                                        <a href="{{ route('messages.attachment.download', [$message, $index]) }}" 
                                           class="flex items-center gap-2 px-3 py-2 rounded-lg {{ $isMine ? 'bg-white bg-opacity-10 hover:bg-opacity-20' : 'bg-gray-100 hover:bg-gray-200' }} transition text-sm">
                        ...
                        <p class="text-xs text-gray-500 mt-1 {{ $isMine ? 'text-right' : '' }}">
                            ...
                            @if($isMine && $message->is_read)
                                <span class="text-blue-600 inline-flex items-center gap-0.5">
        ...
        @empty
            <div class="text-center py-12">
                ...
                <p class="text-sm text-gray-400 mt-2">Envoyez le premier message pour démarrer la conversation</p>
            </div>
        @endforelse
    </div>

    {{-- Formulaire d'envoi --}}
    <form wire:submit.prevent="sendMessage" class="border-t border-gray-200 p-4 bg-white" id="chat-form">
        <div class="space-y-3">
            {{-- Textarea --}}
            <div>
                <textarea 
            ...
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <label class="flex items-center gap-2 cursor-pointer text-gray-600 hover:text-blue-900 transition">
                ...
                <button 
                    ...
                    class="bg-blue-900 hover:bg-blue-800 text-white font-bold px-6 py-3 rounded-full transition transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            <p class="text-xs text-gray-500 flex items-center gap-1">
                <svg class="w-3.5 h-3.5 text-yellow-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                Astuce: Utilisez <kbd class="px-2 py-1 bg-gray-100 rounded text-xs">Ctrl + Entrée</kbd> pour envoyer rapidement
            </p>
        </div>
    </form>
</div>
