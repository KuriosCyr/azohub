        ...
        <div class="container mx-auto px-4 py-12">
            <div class="flex flex-col md:flex-row items-center gap-8">
                ...
                <div class="flex-1 text-center md:text-left">
                    <h1 class="text-4xl font-serif font-medium mb-2">
                        {{ $prestataire->name }}
                    </h1>

                    <div class="flex flex-wrap justify-center md:justify-start items-center gap-4 mb-4 text-ink-300">
                        {{-- Pro/Premium : à part du nom (plutôt que mélangé au texte 4xl), pour ne
                             jamais retomber seul et disproportionné sous un nom énorme sur mobile
                             — regroupé avec les autres indicateurs de statut de cette ligne. --}}
                        @php $plan = $prestataire->currentPlan(); @endphp
                        @if($plan?->slug === 'premium')
                            <span class="flex items-center gap-1 bg-gradient-to-r from-ochre-500 to-ochre-600 text-ink-900 px-3 py-1 rounded-full text-sm font-bold">
                                <x-app-icon name="sparkles" class="w-3.5 h-3.5" />
                                Premium
                            </span>
                        @elseif($plan?->slug === 'pro')
                            <span class="flex items-center gap-1 bg-terracotta-600 text-cream-50 px-3 py-1 rounded-full text-sm font-bold">
