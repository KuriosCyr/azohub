        ...
        <div class="bg-cream-50 rounded-lg border border-ink-100 shadow-sm overflow-hidden">
            <div class="divide-y divide-ink-100">
                @forelse($orders as $order)
                    <div wire:key="order-{{ $order->id }}" class="relative">
                        ...
                        {{-- BUG035 (QA) : le bouton pour livrer une commande n'était accessible
                             qu'en ouvrant la commande puis en faisant défiler jusqu'à sa section
                             "Livrer le travail" — pas trouvé par le testeur. Raccourci direct
                             depuis la liste, pour les commandes qui l'attendent. --}}
                        @if($order->status === 'in_progress')
                            <a href="{{ route('orders.show', $order) }}#livraison"
                               class="absolute right-6 bottom-4 inline-flex items-center gap-1.5 bg-ink-900 hover:bg-ink-700 text-cream-50 text-xs font-bold px-3 py-1.5 rounded-lg transition shadow-sm">
                                <x-app-icon name="upload" class="w-3.5 h-3.5" />
                                Livrer
                            </a>
                        @endif
                    </div>
